...
class RNNLoader:
    ...
    def local_mean_position(self, delta_t):
        ...
        num_particles = self.deck.timestep[rnn.start].particle[
            0].getNumParticles()  # change for more than 1 particle type
        ...
        for timestep in np.arange(self.start_t, self.end_t, delta_t):
            ...
            temp_df = pd.DataFrame(local_mean_pos,
                                   columns=[f"X_{previous_t:.2f}", f"Y_{previous_t:.2f}", f"Z_{previous_t:.2f}"],
                                   index=particle_data.ids)
